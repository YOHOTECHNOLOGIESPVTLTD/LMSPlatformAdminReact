...
import OptionsMenu from 'components/option-menu/index';

// import TableHeader from TableHeader';
import TableHeader from 'features/payment-management/subscriptions/components/SubscriptionHeader'
...
const defaultColumns = [
  {
    ...
    minWidth: 100,
    headerName: 'ID',
    renderCell: ({ row }) => <Typography component={LinkStyled} to="">{`#${row.id}`}</Typography>
  },
  {
    flex: 0.1,
    minWidth: 80,
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        {cardStatsData && (
          <Grid container spacing={2}>
            {cardStatsData.statsHorizontalWithDetails.map((item, index) => {
              return (
