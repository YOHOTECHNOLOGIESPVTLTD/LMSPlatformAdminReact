...
import StepperCustomDot from 'features/institute-management/institute-add-page/components/StepperCustomDot';

import MenuItem from '@mui/material/MenuItem';
import DatePicker from 'react-datepicker';
import StepperWrapper from 'styles/mui/stepper';
import { addInstitute, checkUserName } from 'features/institute-management/services/instituteService';
...
const accountSchema = yup.object().shape({
  ...
  contact: yup.number().required().min(10, 'Please Enter Valid Phone Number')
});
...
const AddInstitutePage = () => {
  ...
  const handleReset = () => {
    ...
    accountReset({ email: '', username: '', name: '', contact: '' });
    personalReset({
      state: '',
      city: '',
      pin_code: Number(''),
  ...
  const onSubmit = async () => {
    ...
    if (activeStep === steps.length - 1) {
      ...
      if (!isUserNameTaken.success) {
        ...
        const result = await addInstitute(data);
        if (result.success) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      aria-describedby="stepper-linear-account-branch_name"
                      {...(accountErrors.branch_name && { helperText: 'This field is required' })}
                    />
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      {...(accountErrors.username && { helperText: accountErrors.username.message })}
                    />
                  )}
                />
              </Grid>
