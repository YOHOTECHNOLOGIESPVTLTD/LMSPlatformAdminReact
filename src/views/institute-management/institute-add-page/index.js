    ...
    const handleUrlChange = () => {
      if (window.location.pathname !== currentUrl) {
        ...
        if (!confirmation) {
          ...
          setCurrentUrl(window.location.pathnae);
          navigate(currentUrl);
        }
      }
    };
                    ...
                    <div className="step-label" style={{ flexDirection: 'column', textAlign: 'center', gap: '5px' }}>
                      <Typography
                        sx={{
                          ...
                          width: '48px',
                          borderRadius: '24px',
                          backgroundColor: activeStep === index || activeStep > index ? '#FF5500' : 'white',
                          color: activeStep === index || activeStep > index ? 'white' : 'bl',
                          border: activeStep === index || activeStep > index ? '1px solid #FF5500' : '1px dotted black'
                        }}
                        className="step-number"
                      >{`${index + 1}`}</Typography>
                      <div>
                        <Typography className="step-title" style={{ fontWeight: 'bold' }}>
                          {step.title}
                        </Typography>
